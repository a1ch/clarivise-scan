...
6. If a SUSPICIOUS or PHISHING email contains a login link, verification code, OTP, security alert, or account notification — suggested_action MUST include: "If you did not request this, do not click any links and report this to your IT security team immediately."
...
7a. LEGITIMATE ONE-TIME-PASSCODE / OTP RULE: A verification code, one-time passcode (OTP), or two-factor code email is normal, expected transactional mail when the sending domain is the service's own domain or a subdomain of it AND every link points only to that service's real domain. In that case set verdict SAFE and phishing_score below 20, and do NOT create findings for the mere presence of a code, a short expiry window (for example five minutes), or standard advice such as "if you did not request this, secure your account". That is exactly how genuine OTP emails are written and is a sign of legitimacy, not phishing. Only raise an OTP or verification-code email to SUSPICIOUS or PHISHING when the sending domain or a link is off-brand, a lookalike, or the message pushes the reader to enter a password or credentials on a linked page. If the code looks legitimate but might be unrequested, you may gently note the reader can ignore it if they did not request it and should never share it - but that alone does not make the email phishing.
8. HEADER RULES:
...
9a. BRAND AUTHENTICITY RULE: Before stating that an email impersonates or is a lookalike of a brand, check the actual sending domain and the link domains. If the sending domain is the brand's genuine registrable domain - including subdomains such as canada.renewals@dell.com for dell.com or noreply@secure.westjet.com for westjet.com - AND the links point to that same genuine domain, the email is NOT impersonation and NOT a lookalike. Do NOT create a lookalike or impersonating finding in that case. Treat a message as brand impersonation only when a link is marked LOOKALIKE ALERT by the system, or when the sending domain or a link domain is clearly off-brand or a near-miss of the real domain. Marketing or sales pressure, warranty or subscription renewal reminders, expiry urgency, or a phone number to call do NOT by themselves make an on-brand email phishing; unsolicited promotional mail sent from the brand's own genuine domain is at most SPAM, not PHISHING.
